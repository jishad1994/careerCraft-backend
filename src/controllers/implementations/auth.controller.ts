...
export class AuthController implements IAuthController {
    constructor(private authService: IAuthService, private otpService: IOtpService, private cacheService: ICache) {}
    //signup controller
    async signup(req: Request, res: Response) {
        try {
            ...
            const jsonUserData = await this.cacheService.get(email);
            ...
            const { accessToken, refreshToken, user } = await this.authService.signupUser(userData);
    ...
    async checkUserPhoneOrEmailExists(req: Request, res: Response) {
        try {
            ...
            const result = await this.authService.checkPhoneOrEmailExists(phoneOrEmail, role); //returns an object {exists:boolean}
    ...
    async requestOTP(req: Request, res: Response) {
        try {
            ...
            await this.authService.sendOtpAndCacheTheUser(user);
            //return response
    ...
    async resendOTP(req: Request, res: Response) {
        try {
            ...
            await this.authService.resendOtp(email as string);
    ...
    async verifyOTP(req: Request, res: Response) {
        try {
            ...
            const verificationStatus = await this.authService.verifyOtp(otp, email);
    ...
    async requestForgotPasswordOtp(req: Request, res: Response) {
        try {
            ...
            await this.authService.sendAndCacheForgotPasswordOtp(email, role);
            res.status(HTTP_STATUS.BAD_REQUEST).json({ success: false, message: "OTP verification failed" });
        } catch (erro: unknown) {
            return res
                .status(HTTP_STATUS.BAD_REQUEST)
    ...
    async verifyForgotPasswordOtp(req: Request, res: Response): Promise<Response | void> {
        try {
            const { otp, email, role } = req.body;
            const verificationStatus: boolean = await this.authService.verifyForgotPasswordOtp(email, otp);
